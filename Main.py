...
# Discord Bot Token
DISCORD_BOT_TOKEN = 'token'  # Replace with your actual token
...
class EnhancedMemoryStore:
    def __init__(self, embedding_model: str = "all-MiniLM-L6-v2"):
        ...
        self.similarity_threshold = 0.85  # Was previously implicit at ~0.5
        
        # Maximum number of memories to return
        self.max_memories = 3  # Reduced from 5
...
class UnifiedConversationHandler:
    ...
    async def process_interaction(
        ...
        try:
            ...
            if interaction_type == "direct_mention":
                messages = [
                ...
                # For voice chat and indirect messages, analyze whether to respond
                ...
                response = analysis.get('response_content', '') if should_respond else None
    ...
    async def _generate_response(self, messages: List[Dict[str, str]]) -> str:
        try:
            response = ollama.chat(
                model='hf.co/neifuisan/Gemma2-9b-Neuro-sama-Q4_K_M-GGUF:latest',
                messages=messages,
                options={
...
New message from {user_id}: {message_content}

As Hikari-chan with Hinata's personality, analyze this message and decide if you want to respond.
Consider if this is a reply to your last message or if it's something you should comment on.

Remember:
- Don't respond to every message
- Check if they're talking to you
- Only join in if you have something meaningful to add
- Stay in character as Hinata from Naruto
- No description actions or roleplaying asterisks
...
class Bot(commands.Bot):
    ...
    async def on_ready(self):
        ...
        self.logger.info('- !test - Join voice and start listening')
            ...
            try:
                ...
                if text:
                    ...
                    asyncio.run_coroutine_threadsafe(
                        self.cog.handle_text(self.current_speaker, text),
                        self.bot.loop
                    )
    ...
    async def on_message(message):
        ...
        if is_direct:
            ...
            response = await bot.conversation_handler.process_interaction(
                user_id=str(message.author),
                guild_id=message.guild.id,
                message_content=scrubbed_content,  # Use scrubbed content
            ...
            logger.info("\nAnalyzing message...")
            decision = await bot.conversation_handler.process_interaction(
                user_id=str(message.author),
                guild_id=message.guild.id,
                message_content=scrubbed_content,  # Use scrubbed content
            ...
            if decision and decision.get('should_respond'):
                response_content = decision.get('response_content', '')
                if response_content:
                    await bot.process_message(message, response_content, use_tts=False)
            else:
                reason = decision.get('reason', 'No reason provided') if decision and isinstance(decision, dict) else 'Invalid decision format'
                logger.info(f"Decided not to respond: {reason}")

        # Only process commands if it's NOT a direct mention and the bot is not responding
        if not is_direct and not (decision and decision.get('should_respond')):
